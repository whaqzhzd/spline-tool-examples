import { color, Layers, v3 } from "cc";
import { Node, Mat4, Vec3, gfx, Color, ModelComponent } from "cc";
...
export function createLineShape(name: string, color?: Color) {
  const {
    ...
    AttributeName,
    updateVBAttr,
  } = window.cce.gizmos.EngineUtils;

  name = name || "Line";
  color = color || Color.WHITE;

  let mesh = MeshUtility.createMesh({
    positions: [v3(0, 0), v3(0, 1), v3(1, 0), v3(1, 1)],
    indices: [0, 1, 1, 2, 2, 3],
...
export function create3DNode(name: string) {
  ...
  node.modelColor = color();
  return node;
}

export function callGizmoFunction(cb: any) {
...
export function getNodeLocalPostion(
  node: Node,
  position: Vec3,
  out?: Vec3,
  isNormal = false
...
export function getNodeWorldPostion(
  node: Node,
  position: Vec3,
  out?: Vec3,
  isNormal = false
