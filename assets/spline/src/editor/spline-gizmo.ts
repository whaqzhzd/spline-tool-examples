...
import PositionController from "../../../../@types/packages/scene/@types/cce/public/gizmos/3d/elements/controller/position-controller";
import { js } from "cc";

if (CC_EDITOR) {
  function findComponentInParent<T extends Component>(
    node: Node,
    ctor: Constructor<T>
  ) {
  ...
  class SplineGizmo extends Gizmo {
    ...
    moveController: PositionController = null!;
    splineLineController: ContinuousLineController = null!;

    currentSplineNodeController: SplineNodeController = null!;
    ...
    get spline(): Spline {
      return js.getClassName(this.target) == "SplineNode"
        ? findComponentInParent(this.target.node, Spline)
        : this.target;
    }
    get splineNodes() {
    ...
    onSplineCurvesUpdate() {
      ...
      for (let i = 0; i < curves.length; i++) {
        ...
        for (let j = 0; j < samples.length; j++) {
          ...
          positions.push(
            getNodeWorldPostion(node, sample.location, pool.Vec3.get())
          );
        }
      }
    ...
    onShow() {
      ...
      if (this.target instanceof SplineNode) {
        this.selectIndex(
          this.splineNodes.indexOf(this.target),
          SplineMoveType.Position
        );
      } else {
    ...
    get currentSelectionIsSpline() {
      ...
      return (
        node && (node.getComponent(Spline) || node.getComponent(SplineNode))
      );
    }
    ...
    updateMoveControllderPos() {
      if (this.moveType === SplineMoveType.Node) {
        this.moveController.setPosition(
          this.spline.node.getWorldPosition(tempVec3)
        );
      } else {
        if (!this.currentSplineNodeController) return;
        if (this.moveType === SplineMoveType.Position) {
          this.moveController.setPosition(
            this.currentSplineNodeController.getSplineNodeWorldPosition()
          );
        } else if (this.moveType === SplineMoveType.Direction) {
          this.moveController.setPosition(
            this.currentSplineNodeController.getSplineNodeWorldDirection()
          );
        } else if (this.moveType === SplineMoveType.InvDirection) {
          this.moveController.setPosition(
            this.currentSplineNodeController.getSplineNodeWorldInvDirection()
          );
        }
      }
    }
      ...
      controller.onControllerMouseMove = () => {
        if (controller.updated) {
          ...
          if (this.moveType === SplineMoveType.Node) {
            ...
            if (this.moveType === SplineMoveType.Position) {
              splineNode.direction = splineNode.direction
                .clone()
                .add(newPos)
                .subtract(splineNode.position);
              splineNode.position = newPos;
              console.log(splineNode.position);
            } else if (this.moveType === SplineMoveType.Direction) {
