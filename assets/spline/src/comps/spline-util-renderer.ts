...
import { Node, _decorator, PrivateNode, Mat4, find, js } from "cc";
...
export default class SplineUtilRenderer extends SplineUtilBase {
  ...
  onEnable() {
    if (this.spline) {
      ...
      if ((this as any)["onNodeListChanged"]) {
        this.spline.nodeListChanged.addListener(
          (this as any)["onNodeListChanged"],
          this
        );
      }
    }
  ...
  onDisable() {
    if (this.spline) {
      ...
      if ((this as any)["onNodeListChanged"]) {
        this.spline.curveChanged.removeListener(
          (this as any)["onNodeListChanged"],
          this
        );
      }
