import { _decorator, Node, Prefab, Quat, Vec3, instantiate, v3, quat } from "cc";
...
export default class NewClass extends SplineUtilRenderer {
  ...
  public compute() {
    ...
    while (distance <= splineCurve.length) {
      ...
      let binormal = Vec3.transformQuat(
        v3(),
        Vec3.RIGHT,
        Quat.fromViewUp(quat(), sample.tangent, sample.up)
      ).normalize();
      let localOffset =
        this.offset + Math.random() * this.offsetRange * Math.sign(this.offset);
