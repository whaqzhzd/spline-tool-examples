...
import {
  ...
  pointInPolygonLineXZ,
  pointPolygonMinDistXZ,
} from "../../utils/mathf";
...
const { ccclass, executeInEditMode, float, type, boolean, property } =
  _decorator;
...
@ccclass("Scatter")
@executeInEditMode
export class Scatter extends SplineUtilRenderer {
  // transform
  @property
  ...
  @property
  get generateCountPerFrame() {
    return this._generateCountPerFrame;
  }
  set generateCountPerFrame(value) {
  ...
  protected _onDirtyChanged(value: any) {
    if (value) {
      this._updateVolume();
  ...
  private _isPosValid(pos: Vec3) {
    ...
    if (this._type === VolumeType.Line) {
      ...
      pointIn =
        dist > this._scatterLineOffset &&
        dist < this.scatterLineOffset + this.scatterLineWidth;
  ...
  _getRandomLinePint(): Vec3 | null {
    ...
    let sample = this.spline.getSampleAtDistance(
      randomRange(0, length),
      tempCurveSample
    );
    ...
    if (this._raycastTerrain) {
      ...
      if (res) {
        Vec3.add(
          randomPos,
          res,
          this._raycastTerrain.node.getWorldPosition(randomPos)
        );
      }
    } else {
      ...
      let results = raycast.raycastAllModels(
  ...
  protected _saveToCache() {
    ...
    saveMesh("_scatter-cache-data_/" + this.uuid + ".mesh", mesh).then(
      (mesh) => {
  ...
  protected _updateVolume() {
    ...
    this._selfVolumeInfo.maxCount =
      this._selfVolumeInfo.volume * this.itemCount;
