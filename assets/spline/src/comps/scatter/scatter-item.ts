...
export default class ScatterItem {
  ...
  init(node: Node, maxCount: number, dataType: ScatterType) {
    ...
    if (this._type === ScatterType.Mesh) {
      ...
      let tempModel =
        tempNode.getComponent(ModelComponent) ||
        tempNode.getComponentInChildren(ModelComponent);
      ...
      for (let i = 0; i < subMeshCount; i++) {
        ...
        this._fixedMeshes[i] = FixedModelMesh.create(
          this._sourceMesh.getVertices(i).length,
          this._sourceMesh.getTriangles(i).length,
          model,
          this.maxCount
        );
        ...
        model.bakeSettings = tempModel!.bakeSettings;
  ...
  shiftStructOffset(offset: number) {
    let structs = this._meshStructs;
    for (let i = 0; i < structs.length; i++) {
      structs[i].primitives.forEach(
        (primitive) => (primitive.indexView!.offset += offset)
      );
      structs[i].vertexBundles.forEach(
        (vertex) => (vertex.view.offset += offset)
      );
    }
  }
  ...
  protected updateMesh(mat: Mat4) {
    ...
    for (let si = 0; si < subCount; si++) {
      ...
      for (let i = 0; i < vertCount; i++) {
        ...
        fixedMesh.writeVertex(
          offset,
          "position",
          Vec3.toArray(
            tempArray3,
            Vec3.transformMat4(tempMeshPos, vert.position, mat)
          )
        );
        fixedMesh.writeVertex(
          offset,
          ...
          Vec3.toArray(
            tempArray3,
            Vec3.transformMat4(tempMeshNormal, vert.position, mat)
          )
        );
        fixedMesh.writeVertex(
          offset,
          ...
          Vec4.toArray(
            tempArray4,
            Vec4.transformMat4(tempMeshTangent, vert.tangent, mat)
          )
        );
        ...
        fixedMesh.writeVertex(
          offset,
          "uv1",
          Vec2.toArray(tempArray2, vert.uv1)
        );
      }
