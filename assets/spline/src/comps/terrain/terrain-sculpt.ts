...
export class TerrainSculpt extends SplineUtilRenderer {
  ...
  calcVolumeHeight(pos: Vec3, h: number) {
  ...
  compute() {
    ...
    let sculptPositionMap: Record<any, any> = {};
    ...
    if (this._volumeType === VolumeType.Line) {
      ...
      for (let j = y1; j <= y2; j++) {
        for (let i = x1; i <= x2; i++) {
          ...
          let t =
            clamp((res.index - 1 + res.t) / (linePoints.length - 1), 0, 1) *
            (this.splineCurve instanceof Spline
              ? this.spline.curves.length
              : 1);
