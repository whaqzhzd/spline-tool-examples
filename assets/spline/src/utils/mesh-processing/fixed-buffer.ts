...
export default class FixedBuffer {
  static create(
    ...
    {
      arrayBuffer,
      arrayBufferVerticesOffset = 0,
      arrayBufferIndicesOffset = 0,
    }: any
  ) {
    let attrs: any = {};
    ...
    if (arrayBuffer) {
      ...
      fixedBuffer._buffer = new ArrayBuffer(
        verticesCount * stride + indicesCount * 2
      );
    }

    fixedBuffer._dataView = new DataView(
      fixedBuffer._buffer,
      fixedBuffer.verticesOffset,
      fixedBuffer.verticesBytes
    );
    fixedBuffer._iView = new Uint16Array(
      fixedBuffer._buffer,
      fixedBuffer.indicesOffset,
      indicesCount
    );
    fixedBuffer._vbuffer = new Uint8Array(
      fixedBuffer._buffer,
      fixedBuffer.verticesOffset,
      fixedBuffer.verticesBytes
    );
    fixedBuffer._ibuffer = new Uint8Array(
      fixedBuffer._buffer,
      fixedBuffer.indicesOffset,
      fixedBuffer.indicesBytes
    );
  ...
  writeVertex(vertOffset: number, attrName: string, value: any) {
  ...
  writeIndex(indexOffset: number, value: any) {
