import { Mesh, Vec3, ValueType, utils, gfx, ModelComponent } from "cc";
...
function getVerticesCount(primitive: any) {
  ...
  for (let name in primitiveAttr) {
    if (primitive[name]) {
      count = Math.min(
        primitive[name].length / (primitiveAttr as any)[name].size,
        count
      );
    }
  }
    ...
    if (!vbuffer || vbuffer.length !== newVBSize) {
      ...
      for (let name in primitiveAttr) {
        if (primitive[name]) {
          updateMeshVB(
            mesh,
            (primitiveAttr as any)[name].gfxName,
            primitive[name],
            vbuffer.buffer,
            vb.stride
          );
