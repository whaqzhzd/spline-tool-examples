...
function _beginFill(meshBender: MeshBender) {
  ...
  _tempHeightRange.set(
    source.minY + meshBender.heightRange.x * source.lengthY,
    source.minY + meshBender.heightRange.y * source.lengthY
  );
}

function _applyVert(
  meshBender: MeshBender,
  vert: MeshVertex,
  sample: CurveSample,
  distanceRatio: number
) {
  vert = MeshVertex.pool.get().set(vert);
  vert.position.y = clamp(
    vert.position.y,
    _tempHeightRange.x,
    _tempHeightRange.y
  );
...
function _endFill(
  meshBender: MeshBender,
  bentVertices: MeshVertex[],
  sampleCache: Map<string, CurveSample>,
  triangles?: number[]
) {
  MeshUtility.updateOrCreateModelMesh(
    meshBender.getComponent(ModelComponent)!,
    {
...
export default class MeshBender extends Component {
  private dirty = false;
  private result: Mesh = null!;
  private useSpline: boolean = false;
  private spline: Spline = null!;
  private intervalStart = 0;
  private intervalEnd = 0;
  private curve: CubicBezierCurve = null!;
  private _sampleCache: Map<string, CurveSample> = new Map();

  private _source: SourceMesh = null!;
  /// <summary>
  ...
  public setInterval1(spline: Spline, intervalStart: number, intervalEnd = 0) {
    ...
    if (
      (intervalEnd != 0 && intervalEnd <= intervalStart) ||
      intervalEnd > spline.length
    ) {
      throw new Error(
  ...
  private fillOnce() {
    ...
    for (let j = 0; j < source.vertices.length; j++) {
      ...
      let distanceRate =
        source.lengthX == 0
          ? 0
          : Math.abs(vert.position.x - source.minX) / source.lengthX;
      let cacheKey = "" + distance;
      let sample: CurveSample = sampleCache.get(cacheKey)!;
      if (!sample) {
        if (!this.useSpline) {
          if (distance > this.curve.length) continue;
          sample = this.curve.getSampleAtDistance(
            distance,
            CurveSample.pool.get()
          );
        } else {
  ...
  private fillRepeat() {
    ...
    let intervalLength = this.useSpline
      ? (this.intervalEnd == 0 ? this.spline.length : this.intervalEnd) -
        this.intervalStart
      : this.curve.length;
    ...
    let remainingLength =
      (this.useSpline ? this.spline.length : this.curve.length) - offset;
  ...
  private fillStretch() {
    ...
    for (let i = 0; i < source.vertices.length; i++) {
      ...
      if (!sample) {
        if (!this.useSpline) {
          sample = this.curve.getSampleAtDistance(
            this.curve.length * distanceRate,
            CurveSample.pool.get()
          );
        } else {
          let intervalLength =
            this.intervalEnd == 0
              ? this.spline.length - this.intervalStart
