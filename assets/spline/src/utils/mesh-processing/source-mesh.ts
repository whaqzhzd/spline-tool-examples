import { Mesh, Vec3, Quat, gfx, Vec4, Vec2, v3, quat } from "cc";
...
export default class SourceMesh {
  ...
  private buildData() {
    ...
    for (let si = 0; si < subMeshCount; si++) {
      ...
      let positions = this._mesh.readAttribute(
        si,
        gfx.AttributeName.ATTR_POSITION
      )!;
      let normals = this._mesh.readAttribute(si, gfx.AttributeName.ATTR_NORMAL)!;

      let tangents = this._mesh.readAttribute(
        si,
        gfx.AttributeName.ATTR_TANGENT
      )!;
      ...
      let uv1s = this._mesh.readAttribute(
        si,
        gfx.AttributeName.ATTR_TEX_COORD1
      )!;
